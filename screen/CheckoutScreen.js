    ...
    async onSave() {
        ...
        await Helper.post(BASEURL + CHECK_URL, formData, header, async (results) => {
            if (results.status == 'SUCCESS') {
                ...
                await props.CheckTypeControll(typetmp == 'O' ? true : false)
        ...
        return (
            <View style={[styles.center, { paddingLeft: 4 }]}>
                <TouchableOpacity style={{ width: 35, height: 35, alignItems: 'center', justifyContent: 'center' }}
                    ...
                    <Icon name='arrow-left' size={28} color='#F1948A' />
                </TouchableOpacity>
            </View>
        );
    }
    ...
    ComponentCenter = () => {
        ...
        return (
            <View style={[styles.center]}>
                <Text style={[styles.bold, { color: 'white', fontSize: 26 }]}>{`ลงเวลาออกงาน`}</Text>
            </View>
        );
    }
        ...
        return (
            <View style={{ flex: 1 }}>
                ...
                <View style={{ flex: 1, alignItems: 'center' }}>
                    ...
                    <Text style={[styles.bodythai, { fontSize: 24, color: darkColor, width: '100%', textAlign: 'center' }]}>{`${props.userInfo.position}`}</Text>
                    <View style={[styles.center]}>
                        <TouchableOpacity disabled={this.state.check} style={[styles.buttonCheck, styles.shadow, styles.center, { backgroundColor: this.state.check != true ? darkColor : 'gray' }]}
                            ...
                            <Text style={[styles.bodythai, { fontSize: 26, color: 'white' }]}>{`กดปุ่มเพื่อลงเวลาออกงาน`}</Text>
